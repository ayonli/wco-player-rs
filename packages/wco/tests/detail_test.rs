use wco::{get_series_detail, get_video_info, search_series};
...
async fn test_family_guy() {
    ...
    let info = get_video_info(&detail.episodes[0].url, None).await.unwrap();
...
async fn test_futurama() {
    ...
    let last_episode = detail.episodes.last().unwrap();
