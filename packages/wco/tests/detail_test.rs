...
async fn test_the_penguins_of_madagascar() {
    let series = search_series("The Penguins of Madagascar", None).await.unwrap();
    assert!(!series.is_empty());
    ...
    println!("✅ The Penguins of Madagascar - Got video info: {}", info.filename);
}
...
async fn test_video_info_structure() {
    ...
    assert!(info.filename.ends_with(".mp4"), "Filename should end with .mp4");
    ...
    if let Some(fhd_url) = &info.full_hd_url {
        assert!(fhd_url.starts_with("http"), "Full HD URL should be valid HTTP(S)");
    }
