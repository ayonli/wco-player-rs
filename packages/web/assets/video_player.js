...
import { loadAppState, setPlaybackPosition, updateSeriesEpisodeAndPosition, updateUrlHash, } from "./state_manager";
/**
    ...
    playbackProgressInterval = setInterval(() => {
        ...
        if (video.readyState >= 1 && !video.paused && !video.ended) {
            ...
            if (Math.abs(currentTime - lastSavedPosition) > 1) {
                ...
                const state = loadAppState();
                updateSeriesEpisodeAndPosition(state?.series || null, state?.episode || null, currentTime);
            }
        }
        else if (video.ended) {
            ...
            const state = loadAppState();
            updateSeriesEpisodeAndPosition(state?.series || null, state?.episode || null, null);
        }
    }, 2000);
    // Handle video end
