 ...
 */
export function savePlayerState(series, episode, playbackPosition = 0) {
...
function getEpisodeListContainer() {
    return document.querySelector(".episode-list-content");
}
function getSelectedEpisodeElement() {
    return document.querySelector(".episode-item.selected");
}
function getEpisodeItemElement(episodeUrl) {
    return document.querySelector(`.episode-item[data-episode-url="${CSS.escape(episodeUrl)}"]`);
}
function getVideoElement() {
    return document.querySelector("video.video-element");
}
/**
 ...
 */
function setupAutoScrollOnSelection() {
 ...
 */
export function setFullscreenMode(isFullscreen) {
    const playerPage = document.getElementById("player-page");
    if (!playerPage) {
        return;
    }
 ...
 */
function toggleFullscreenStyles() {
    const playerPage = document.getElementById("player-page");
    if (!playerPage) {
        return;
    }
        ...
        if (video.readyState >= 1 && !video.paused && !video.ended) {
            ...
            if (Math.abs(currentTime - lastSavedPosition) > 1) {
                ...
                if (state?.series && state?.episode) {
                    savePlayerState(state.series, state.episode, currentTime);
                }
            }
        }
        else if (video.ended) {
